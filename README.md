...
`learn-business-deeply` 是一个面向实习和真实项目学习场景的 Codex Skill。它以技术工程原理为学习主线，以本地代码、配置、日志、数据库结构、项目文档和任务历史作为解释材料与事实证据，帮助学习者理解陌生技术的核心抽象、运行机制、工程权衡、故障边界以及项目采用方式。

这个 Skill 不把学习过程处理成零散知识点或业务文件的罗列。它会先展示由技术原理组织的知识地图并诊断学习者已有的理解，再按照“技术问题 → 核心抽象 → 工程原理 → 项目实现映射 → 工程权衡与失败边界 → 面试抽象与迁移”展开。项目代码可以在相关原理节点中穿插，用于说明项目怎样采用和验证该机制，但不会默认取代技术主线。

## 核心能力

- 使用阶段标识持续展示当前学习进度，长对话中也能判断正在学习哪条主线和哪个知识节点。
- 以技术工程原理组织知识地图，学习深度默认覆盖项目理解、方案分析和常见故障排查，不主动进入源码级实现。
- 在相关原理节点中穿插必要的项目代码、配置和日志，用真实实现帮助理解和验证原理，而不是默认还原完整业务调用链。
...
- 将追问定位到前置、当前、后续或独立旁支；地图外知识会同步进入当前任务的学习面板。
- 每完成一个相对完整的知识节点，在常规检测题之外增加一道只使用本轮已讲知识的拓展思考题，通过少量提示引导条件变化、机制组合、反向诊断、方案权衡或场景迁移。
...
```text
$learn-business-deeply 我不了解 Redis。请先建立工程原理知识地图，再结合当前项目说明相关机制的采用方式。
```
...
学习过程中可以持续追问，也可以告诉 Codex“这是重点”或指出自己的疑点。后续知识会先获得解决当前困惑所需的简要解释，并注明未来详细展开的知识节点；前置知识缺口会立即解释；地图外问题会同步更新到当前任务的学习面板。当一个主题讨论充分后，可以明确提出：
